...
export function MetricCards({ summary, backtest, avgAlpha }: MetricCardsProps) {
  ...
  return (
    <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 lg:grid-cols-5 gap-2 sm:gap-3 py-4">
      ...
      />
      <Metric label="Best" value={`${backtest.best_return > 0 ? "+" : ""}${backtest.best_return}%`} color="text-bullish" glow="glow-green" />
      <Metric label="Worst" value={`${backtest.worst_return}%`} color="text-bearish" glow="glow-red" />
